...
def physical_mask(samples, idx):
    ...
    if 'mass_1' in idx and 'mass_2' in idx:
        m1 = s[:, idx['mass_1']]
        m2 = s[:, idx['mass_2']]
        m &= (m1 > m2) & (m2 > 0)
...
PARAMS15 = ['mass_1', 'mass_2', 'q', 'redshift', 'luminosity_distance', 'theta_jn', 'ra', 'dec',
                     'psi', 'phase', 'geocent_time', 'a_1', 'a_2','tilt_1','tilt_2'] # 15 parameters

PARAMS13 = ['mass_1', 'mass_2', 'luminosity_distance', 'theta_jn', 'ra', 'dec',
                     'psi', 'phase', 'geocent_time', 'a_1', 'a_2','tilt_1','tilt_2'] # 13 parameters

PARAMS11 = ['mass_1', 'mass_2', 'luminosity_distance', 'theta_jn', 'ra', 'dec',
...
means11 = np.loadtxt('Errors_LISA_MBHB_catalog_popIII_withfisher_tilts_Mzmin103_Mzmax108_SNR0.txt', skiprows=1, usecols=keep)
...
invfisher_13d = np.load("inv_fisher_matrices_LISA_MBHB_catalog_popIII_withfisher_tilts_Mzmin103_Mzmax108_SNR0.npy")  # shape (N, 13, 13)
...
z_vals = np.array([z_at_value(Planck18.luminosity_distance, d, zmin=1e-9, zmax=50)
...
#Parameters when skipping tilt1, and tilt2 from the multivariate sampling
inj15[:, 0]  = samples11[:, 0]                  # mass_1_source
inj15[:, 1]  = samples11[:, 1]                  # mass_2_source
...
df.to_csv("mbhb_popIIInoisy_catalog_Mzmin103_Mzmax108_pars.tsv", sep="\t", index=False)
...
df = pd.DataFrame(snr)
df.to_csv("mbhb_popIIInoisy_catalog_Mzmin103_Mzmax108_snr.tsv", sep="\t", index=False)
...
#fisher_parameters = ['mass_1_source', 'mass_2_source', 'luminosity_distance', 'theta_jn', 
#                     'psi', 'phase', 'a_1', 'a_2']
...
gw.fishermatrix.analyze_and_save_to_txt(network = network,
                                        ...
                                        population_name = f'MBHB_catalog_popIII_noisy_signal_with_fishertilts_Mzmin103_Mzmax108',
                                        waveform_model = waveform_model,
                                        f_ref = 1e-4,
